...
OUTPUT_DIR = ROOT / "outputs" / "consistency_runs"
RUN_COUNT = 5
SLEEP_SECONDS = 2
...
def main() -> int:
    ...
    print(f"[consistency] Writing artifacts to {OUTPUT_DIR}")
    ...
    for run_number in range(1, RUN_COUNT + 1):
        ...
        env["RUN_ID"] = run_id
        print(f"[consistency] Starting {run_id}")
        ...
        if proc.returncode == 0:
            latest_pdf = ROOT / "outputs" / "final_report.pdf"
            ...
            print(f"[consistency] Completed {run_id}")
        else:
            summary_lines.append(
                f"{run_id}: failed (exit {proc.returncode}) | elapsed_seconds={elapsed_seconds:.3f} | log={run_log.name}"
            )
            print(f"[consistency] {run_id} failed with exit code {proc.returncode}")
    ...
    print(f"[consistency] Summary written to {summary_path}")
    return 0
